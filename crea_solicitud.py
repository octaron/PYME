...
class PDF(FPDF):
    ...
    def header(self):
        # Logo
        self.image('C:/Users/octavio.salazar/Desktop/MiPYME/static/small_logo.png',15,10,50)
        # Arial bold 15
        self.set_font('Arial', 'B', 8)
        # Calcular ancho del texto (title) y establecer posici�n
    ...
    tamano_fuente):
        ...
        #Color de texto
        self.set_text_color(0,0,0)
        self.cell(epw/3, 2*th, 'Apellido paterno: '+ ap_paterno,0)
        self.cell(epw/3, 2*th, 'Apellido materno: '+ ap_materno,0)
        ...
        #self.cell(epw/3, 2*th, 'Pa�s de nacimiento: ',0)
        self.cell(epw/3, 2*th, 'Fecha de nacimiento: '+fec_nacimiento,0)
        ...
        #self.cell(epw/4, 2*th, 'G�nero: ',0)
        #self.ln(2*th)

        self.cell(epw/3, 2*th, 'Domicilio actual calle: '+calle,0)
        self.cell(epw/3, 2*th, 'Número ext: '+n_ext,0)

        if len(n_int) ==0:
            n_int='-'

        self.cell(epw/3, 2*th, 'Número int: '+n_int,0)
        #self.cell(epw/4, 2*th, 'Ciudad: ',0)
        self.ln(2*th)

        self.cell(epw/3, 2*th, 'Colonia: '+asentamiento,0)
        self.cell(epw/3, 2*th, 'Alcaldía o municipio: '+municipio,0)
        self.cell(epw/3, 2*th, 'Estado: '+estado,0)
        ...
        #self.cell(epw/4, 2*th, 'Tel�fono con lada: ',0)
        self.cell(epw/3, 2*th, 'Teléfono: '+tel,0)
        self.cell(epw/3, 2*th, 'e-mail personal: '+email,0)
        #self.cell(epw/4, 2*th, 'e-mail laboral: ',0)
        self.ln(2*th)

        #self.cell(epw/3, 2*th, 'Tipo de vivienda: ',0)
        #self.cell(epw/3, 2*th, '# dependientes econ�micos: ',0)
        #self.cell(epw/3, 2*th, 'A�os residencia: ',0)
        #self.ln(2*th)

        PDF.crea_mapa(calle +' ' + n_ext+ ' ' +n_int+' ' +asentamiento+ ' '+ municipio+ ' '+ estado , 'ubicacion_'+rfc)
        print(calle +' ' + n_ext+ ' ' +n_int+' ' +asentamiento+ ' '+ municipio+ ' '+ estado)

        self.cell(epw/3, 2*th, 'Referencias de ubicación: ',0)
        #self.ln(2*th)
        self.image('C:/Users/octavio.salazar/Desktop/MiPYME/mapa_ubicacion/ubicacion_'+rfc+'.PNG',75,None,80)
        self.ln(2*th)
    def info_empleo(self,title_empleo,tamano_fuente):
        ...
        #Color de texto
        self.set_text_color(0,0,0)
        self.cell(epw/2, 2*th, 'Nombre del negocio:',0)
        self.ln(2*th)
        ...
        #self.cell(epw/3, 2*th, 'Departamento o �rea en la que labora:',0)
        ...
        #self.cell(epw/4, 2*th, 'CP:',0)
        #self.cell(epw/4, 2*th, 'N�mero de empleado:',0)
        self.cell(epw/3, 2*th, 'Ciudad o estado:',0)
        self.cell(epw/3, 2*th, 'Colonia:',0)
        self.cell(epw/3, 2*th, 'Alcaldía o municipio:',0)
        self.ln(2*th)
...
tamano_fuente = 7
pdf.print_datos_credito('INFORMACI�N DEL SOLICITANTE',
                        'INFORMACI�N DEL EMPLEO',
                        'REFERENCIAS DEL EMPLEO',
                        'REFERENCIAS BANCARIAS(CUENTA DE N�MINA)',
